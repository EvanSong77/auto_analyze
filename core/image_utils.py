...
async def analyze_image(image_data: Union[str, bytes], image_format: str = "png",
                        prompt: str = "为了进行数据分析任务我制作一张图片。请描述图片的内容。只要包含图片中关于数据的内容，不要有进一步的推断。",
                        is_base64: bool = True) -> str:
    """
    使用视觉模型分析图片内容
...
if __name__ == "__main__":
    ...
    async def main():
        try:
            test_image_path = r"D:\codewen_workspace\AIGC_Work\auto_analyze\data\3月份产品业绩分析.png"  # 替换为实际测试图片路径
